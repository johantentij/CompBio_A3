...
def makeAntioboticConcentrations(transitionRatio=.1):
    antibiotic[:, :] = 1e-2
    for i in range(1, steps):
        prevConcentration = baseConcentration * (i - 1) + 1e-2
        targetConcentration = baseConcentration * i + 1e-2

        interConcentration = prevConcentration + (targetConcentration - prevConcentration) * transitionRatio
        antibiotic[:, i * N_y // steps - transitionWidth:] = interConcentration
        antibiotic[:, i * N_y // steps:] = targetConcentration

        detectorThresholds[i] = .5 * (i * N_y // steps - transitionWidth + (i + 1) * N_y // steps)

makeAntioboticConcentrations(transitionRatio=0.5)
...
# im2 = ax2.imshow(gene_map, cmap="rainbow", origin='lower', vmin=K_antibiotic[0], vmax=80)
...
for ratio in ratios:
    ...
    makeAntioboticConcentrations(ratio)
    bacteria_indices = np.empty((N_x * N_y, 2), dtype=np.int32)
    bacteria_indices[0] = [N_x // 2, 0]
    aliveCount = 1
    ...
    for i in range(max_iterations):
        ...
        if (np.sum(bacteria[:, detectorThresholds[furthest]:]) > 0):
    ...
    meanTimes.append(1 / np.mean(interTimes))

plt.plot(ratios, meanTimes)

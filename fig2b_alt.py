...
import CA_numba as ca_numba


def _build_antibiotic_numba(middle_conc):
    """
    Convert Fig2B antibiotic bands to CA_numba format (Nx,Ny).
    """
    levels = np.array([0, middle_conc, 3000, middle_conc, 0], dtype=float) * MIC_base
    antibiotic = np.zeros((Nx, Ny))

    zone_width = Nx // len(levels)

    for z, level in enumerate(levels):
        x_start = z * zone_width
        x_end = (z + 1) * zone_width if z < len(levels) - 1 else Nx
        antibiotic[x_start:x_end, :] = level

    return antibiotic


def _front_x_left_numba(bacteria):
    """
    Determine rightmost occupied column in left half of plate.
    CA_numba uses (Nx,Ny) indexing.
    """
    half = bacteria[: Nx // 2, :]
    if not half.any():
        return 0
...
def run_ca_single(middle_conc, seed=None):
    ...
    # initialize CA_numba globals
    nutrients = ca_numba.initNutrient * np.ones((ca_numba.N_x, ca_numba.N_y))
    bacteria = np.zeros((ca_numba.N_x, ca_numba.N_y), dtype=np.bool_)

    bacteria_indices = np.empty((ca_numba.N_x * ca_numba.N_y, 2), dtype=np.int32)
    K_antibiotic = np.empty(ca_numba.N_x * ca_numba.N_y)

    genomeIDs = np.empty((ca_numba.N_x * ca_numba.N_y, 4), dtype=np.int32)

    # seed bacterium
    bacteria_indices[0] = [ca_numba.N_x // 2, 0]
    bacteria[bacteria_indices[0,0], bacteria_indices[0,1]] = True

    aliveCount = 1
    genomeCounter = 0

    K_antibiotic[0] = MIC_base
    genomeIDs[0] = [0,0,bacteria_indices[0,0],bacteria_indices[0,1]]

    antibiotic = _build_antibiotic_numba(middle_conc)

    target_x = highest_band_x(middle_conc)

    for step in range(MAX_STEPS):

        nutrients = ca_numba.updateNutrients(nutrients)
        nutrients[bacteria] -= ca_numba.nutrientConsumption

        aliveCount, genomeCounter = ca_numba.bacteriaStep(
            nutrients,
            bacteria,
            bacteria_indices,
            antibiotic,
        ...
        if step % CHECK_EVERY == 0:
            ...
            front = _front_x_left_numba(bacteria)
...
def calibrate_ca():
    ...
    antibiotic = np.zeros((ca_numba.N_x, ca_numba.N_y))
    ...
    for step in range(MAX_STEPS):
        ...
        if step % CHECK_EVERY == 0:
            ...
            if front >= CALIB_TARGET_X:
                print(f"  CA calibration: front reached x={CALIB_TARGET_X} at step {step}")
                return step
